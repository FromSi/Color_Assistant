...
class ComboModelImpl : ComboModel {
    ...
    override fun getValue(): MutableList<String> {
        ...
        for (i in 0 until colorList.size)
            answerList.add(ColorConverter.getFullAnswer(colorList[i]))

        return answerList
    }

    override fun initColorList(colorList: MutableList<String>) {

        for (i in 0 until colorList.size)
            this.colorList.add(Color.parseColor(colorList[i]))
    }
}